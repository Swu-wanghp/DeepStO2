...
def train(agrs):
    # 用于计算准确率平均值
    averageAll = 0
    # outputPath 为中间输出保存路径,命名方式：train_batchsize  lr
    outputFilename = "results" + "trainBatchsize" + "_" + agrs.train_batchsize + "_" + agrs.lr + ".txt"
    outputPath = os.path.join("./results", outputFilename)
    ...
    for p in range(args.sub_num):
        ...
        train_loader = DataLoader(dataset=train_dataset, batch_size=args.train_batchsize, num_workers=16, shuffle=True)
        val_loader = DataLoader(dataset=test_datast, batch_size=args.test_batchsize, num_workers=16, shuffle=False)
